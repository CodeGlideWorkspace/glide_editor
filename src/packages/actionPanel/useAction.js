...
class Action {
   ...
   * @param {Object} config 节点定义
   *
   * @returns {Object<{ [eventName]: Handle }>}
   */
  render(node, config) {
    return node?.config?.actions?.reduce((props, action) => {
      const { eventName, actions } = action
      const eventDefinition = config?.eventDefinitions?.find((item) => item.name === eventName)
      if (!eventDefinition) {
        return props
      }

      props[eventName] = this.createEvent(node.code, actions, eventDefinition)
      return props
    }, {})
  }
